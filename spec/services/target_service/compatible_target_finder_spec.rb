...
describe 'Services::TargetService::CompatibleTargetFinder', type: :service do
  ...
  context 'when target matches the criteria - north targets' do
    ...
    it 'creates conversations with the matched users' do
      expect { subject.call }.to change(Conversation, :count).by(3)
    end
  end

  context 'when target matches the criteria but conversations alrealdy exist' do
    let!(:target) { create(:target, :with_north_location, topic: topic, radius: 2000.0) }
    let!(:receivers) { north_targets.map(&:user) }

    before do
      ConversationService::Creator.new(sender: target.user, receivers: receivers, topic: topic).call
    end
    ...
    it 'does not create any conversations' do
      expect { subject.call }.to change(Conversation, :count).by(0)
    end
  end
  ...
  context 'when target matches the criteria - all targets' do
    ...
    it 'creates conversations with the matched users' do
      expect { subject.call }.to change(Conversation, :count).by(6)
    end
  end
